...
> 简体中文文档索引（English、繁體中文版本见页面底部语言切换菜单）
...
- [隐藏标记与特殊标记](./designs/hidden-and-special-flags.md) — `hidden_flag` 数据级防剧透 + `special_flag` 位掩码过滤
...
本站提供 English / 简体中文 / 繁體中文 三种语言，请使用页面底部的语言
切换菜单（偏好会保存在浏览器中，链接可通过 `?lang=` 参数分享）。
