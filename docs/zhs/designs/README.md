...
> [← 返回总索引](../SUMMARY.md) · [架构概览](../guides/architecture.md)
...
| 文档 | 主题 | 核心问题 |
| --- | --- | --- |
| [打点审批工作流](./punctuate-workflow.md) | `punctuate` 提交→审核→晋升状态机 | 玩家打点如何安全地变成全服可见的正式点位，又如何防破坏 |
...
这三块是整个 Rust 后端里最「业务驱动」的部分，恰恰也是最容易在新人 review
时被误改成「看起来更通用、实际破坏了游戏语义」的部分：

- 打点状态机里 `Rejected` 不是终态而是「可重新提交」的中间态，随手改成
