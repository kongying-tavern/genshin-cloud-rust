...
`MARKER_PAGE_SIZE = 3000`（`marker_doc.rs:17`）是在**页数（请求次数）**和**单页下载量**
之间取的平衡：
...
Tag（图标分类）同样是单 blob。这两类数据量小、且整体性强（拆开没意义），所以一个
MD5 搞定。
...
`BinaryMd5Vo`（`binary_doc.rs:17`）的 `time` 字段是这一批页的生成时间戳（毫秒），
...
`MarkerLinkage` / Tag 单 blob，清单里只有一个 MD5（用 `all_list_bin_md5` 而非
`list_page_bin_md5` 命名以示区别）。

## 5. Java vs Rust：缓存层是当前的主要差距
...
Rust 侧当前**每次都重新生成**——`item_doc.rs:44` 的注释明确写了这一点：
...
> handler regenerates on demand.

也就是说，Rust 的 `list_page_bin/{md5}` 现在要重新查全表、重新分组、重新压缩、
逐一比对 MD5 找到匹配页才返回。这在数据量小时尚可，但 marker 全量压缩每请求一次
是不可接受的生产开销。**待办**是引入进程内缓存（候选方案：`moka` LRU 或 Redis），
键为 MD5、值为压缩字节，复刻 Caffeine 的「按 MD5 寻址、显式失效」语义。失效触发点
和 Java 侧一致：点位 / 物品 / 关联发生写操作后，由 `cache.rs` 的域刷新端点驱逐对应
缓存（见 [架构概览](../guides/architecture.md#redis-与-minio-集成点)）。
...
| Caffeine `neverRefreshCacheManager` | **未实现**（每次重新生成，待加缓存） |
...
- **进程内缓存**：最紧迫的待办，见 [第 5 节](#5-java-vs-rust缓存层是当前的主要差距)。

候选 `moka`（进程内 LRU）或 Redis（跨实例共享）；多副本部署时 Redis 更稳。

- **`list_page_bin` 的扫描成本**：即便加了缓存，首次 miss 时仍要全表扫描 + 全量压缩。

可考虑把「MD5 → 压缩字节」的映射在数据写入时就预算好落库，避免读时计算。

- **MD5 清单的生成时间**：当前 `time` 用请求当下的时间戳，导致每次调 `list_page_bin_md5`

返回的 `time` 都变（哪怕数据没变）。应改为数据实际变更时间，让客户端能判断「真的
没更新」。
