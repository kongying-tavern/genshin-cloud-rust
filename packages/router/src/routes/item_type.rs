...
use _utils::schemas::item_type::Schema as ItemTypeSchema;
...
pub async fn register() -> Result<Router> {
    let router = Router::new()
        .route(
            ...
            post(|Path(_is_search_self): Path<String>| async move {
        ...
        .route(
            "/update",
            post(|Json(_frm): Json<ItemTypeSchema>| async move {
                // 修改物品类型
                ""
            }),
        )
        .route(
            "/add",
            put(|Json(_frm): Json<ItemTypeSchema>| async move {
        ...
        .route(
            "/move/:id",
            post(
                |Path(_id): Path<String>, Json(_frm): Json<Vec<i64>>| async move {
