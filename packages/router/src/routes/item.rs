...
pub async fn register() -> Result<Router> {
    let router = Router::new()
        ...
        .route(
            "/update/:is_edit_same",
            post(
                |Path(_is_edit_same): Path<String>, Json(_frm): Json<ItemSchema>| async move {
                    // 修改物品
        ...
        .route(
            "/join/:type_id",
            post(
                |Path(_type_id): Path<String>, Json(_frm): Json<Vec<i64>>| async move {
        ...
        .route(
            "/add",
            put(|Json(_frm): Json<ItemSchema>| async move {
        ...
        .route(
            "/copy/:area_id",
            put(
                |Path(_area_id): Path<String>, Json(_frm): Json<Vec<i64>>| async move {
